...
std::string TransferRoute::GetInfo() const {
    std::string result = "A route " + from_city_title + " - " + to_city_title + " with " +
                         std::to_string(segments.size() - 1) + " transfers" + '\n';
    for (int i = 0; i < segments.size(); ++i) {
        ...
        if (i + 1 < segments.size())
            result += "\t- Transfer from " + segments[i].transfer_after->transfer_from_title + " to " +
